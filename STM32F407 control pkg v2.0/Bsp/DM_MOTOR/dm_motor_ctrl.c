...
void dm_motor_init(void)
{
  ...
  motor[Motor2].ctrl.kp_set   = 3.5f;
	motor[Motor2].ctrl.kd_set 	= 3.0f;
	motor[Motor2].tmp.PMAX		= 310.0f;
	motor[Motor2].tmp.VMAX		= 30.0f;
	motor[Motor2].tmp.TMAX		= 10.0f;
  // 设置Motor3的电机信息  DM3519电机0x03
  ...
  motor[Motor3].ctrl.kp_set   = 3.5f;
	motor[Motor3].ctrl.kd_set 	= 3.0f;
	motor[Motor3].tmp.PMAX		= 310.0f;
	motor[Motor3].tmp.VMAX		= 30.0f;
	motor[Motor3].tmp.TMAX		= 10.0f;
  // 设置Motor4的电机信息  DM3519电机0x04
  ...
  motor[Motor4].ctrl.kp_set   = 3.5f;
	motor[Motor4].ctrl.kd_set 	= 3.0f;
	motor[Motor4].tmp.PMAX		= 310.0f;
	motor[Motor4].tmp.VMAX		= 30.0f;
	motor[Motor4].tmp.TMAX		= 10.0f;
  */
  /* v2版本的电机参数，主要是kp和kd的调整，经过机械结构安装后调试得出*/
  // 设置Motor1的电机信息  DM4310电机
	motor[Motor1].id = 0x01;
	motor[Motor1].mst_id = 0x11;
	motor[Motor1].tmp.read_flag = 1;
	...
	motor[Motor1].ctrl.pos_set 	= 0.0f;
	motor[Motor1].ctrl.cur_set 	= 0.03f;
  motor[Motor1].ctrl.kp_set   = 80.0f;
	motor[Motor1].ctrl.kd_set 	= 0.8f;
	motor[Motor1].tmp.PMAX		= 12.5f;
	motor[Motor1].tmp.VMAX		= 30.0f;
	motor[Motor1].tmp.TMAX		= 10.0f;
  ...
  // 设置Motor2的电机信息  DM3519电机0x02
	...
	motor[Motor2].ctrl.pos_set 	= 0.0f;
	motor[Motor2].ctrl.cur_set 	= 0.03f;
  motor[Motor2].ctrl.kp_set   = 7.0f;
	motor[Motor2].ctrl.kd_set 	= 1.2f;
	motor[Motor2].tmp.PMAX		= 310.0f;
	motor[Motor2].tmp.VMAX		= 30.0f;
	motor[Motor2].tmp.TMAX		= 10.0f;
  ...
  // 设置Motor3的电机信息  DM3519电机0x03
	...
	motor[Motor3].ctrl.pos_set 	= 0.0f;
	motor[Motor3].ctrl.cur_set 	= 0.03f;
  motor[Motor3].ctrl.kp_set   = 7.0f;
	motor[Motor3].ctrl.kd_set 	= 1.2f;
	motor[Motor3].tmp.PMAX		= 310.0f;
	motor[Motor3].tmp.VMAX		= 30.0f;
	motor[Motor3].tmp.TMAX		= 10.0f;
  ...
  // 设置Motor4的电机信息  DM3519电机0x04
	...
	motor[Motor4].ctrl.pos_set 	= 0.0f;
	motor[Motor4].ctrl.cur_set 	= 0.03f;
  motor[Motor4].ctrl.kp_set   = 7.0f;
	motor[Motor4].ctrl.kd_set 	= 1.2f;
	motor[Motor4].tmp.PMAX		= 310.0f;
	motor[Motor4].tmp.VMAX		= 30.0f;
	motor[Motor4].tmp.TMAX		= 10.0f;
}
...
void can1_rx_callback(void)
{
  char strTem[8]; 
	uint16_t rec_id;
	canx_receive(&hcan1, &rec_id);
	if(g_start_tick>=200 && g_yemian_num == 1){                                            // 启动调度后2s，并处于页面一才开始接收数据
    switch (rec_id)
    {
      case 0x11: 
        dm_motor_fbdata(&motor[Motor1], xCAN1.RxData); receive_motor_data(&motor[Motor1], xCAN1.RxData);
        g_dm4310_status = motor[Motor1].para.state;
        Judge_DMmotor_status(g_dm4310_status, 110, 118);
        g_dm4310_real_pos = motor[Motor1].para.pos;
        sprintf(strTem, "%.2f", g_dm4310_real_pos*180/3.14f);                            // 使用sprintf进行字符串格式化，输出整形数据
        LCD_String(120, 151-8, strTem, 16, WHITE, BLACK);  
        break;
      case 0x12: 
        dm_motor_fbdata(&motor[Motor2], xCAN1.RxData); receive_motor_data(&motor[Motor2], xCAN1.RxData);
        g_dm3910_1_status = motor[Motor2].para.state;
        Judge_DMmotor_status(g_dm3910_1_status, 110, 164);
        g_dm3910_1_real_pos = motor[Motor2].para.pos;
        sprintf(strTem, "%.2f", g_dm3910_1_real_pos*187/19.95/3.14f);                    // 使用sprintf进行字符串格式化，输出整形数据
        LCD_String(120, 197-8, strTem, 16, WHITE, BLACK);                                // 目前这里输出的结果是经过减速器输出的角度
        break;
      case 0x13:
        dm_motor_fbdata(&motor[Motor3], xCAN1.RxData); receive_motor_data(&motor[Motor3], xCAN1.RxData);
        g_dm3910_2_status = motor[Motor3].para.state;
        Judge_DMmotor_status(g_dm3910_2_status, 110, 210);
        g_dm3910_2_real_pos = motor[Motor3].para.pos;
        sprintf(strTem, "%.2f", g_dm3910_2_real_pos*187/19.95/3.14f);                    // 使用sprintf进行字符串格式化，输出整形数据
        LCD_String(120, 243-8, strTem, 16, WHITE, BLACK);                                // 目前这里输出的结果是经过减速器输出的角度
        break;
      case 0x14:
        dm_motor_fbdata(&motor[Motor4], xCAN1.RxData); receive_motor_data(&motor[Motor4], xCAN1.RxData);
        g_dm3910_3_status = motor[Motor4].para.state;
        Judge_DMmotor_status(g_dm3910_3_status, 110, 256);
        g_dm3910_3_real_pos = motor[Motor4].para.pos;
        sprintf(strTem, "%.2f", g_dm3910_3_real_pos*187/19.95/3.14f);                    // 使用sprintf进行字符串格式化，输出整形数据
        LCD_String(120, 289-8, strTem, 16, WHITE, BLACK);                                // 目前这里输出的结果是经过减速器输出的角度
        break;
    }
  }
}
