 ...
 * 版权归于林楚然，盗版必究
 * 温馨提示：不要看CubeMX的配置文件，更不要使用CubeMX更新，不然代码会被刷没，并且无法烧录
******************************************************************************/
/* Includes ------------------------------------------------------------------*/
#include "main.h"
#include "cmsis_os.h"
...
uint8_t volatile g_yemian_num = 1;             // 当前所在的页面，默认上电进入页面一，1-页面一（电机点动调试），2-页面二（逆运动学控制）
uint16_t volatile g_moveplate_x = 120;             // 动平台绕着竖直轴旋转角度，初始化为120，屏幕正中心
uint16_t volatile g_moveplate_y = 200;             // 动平台绕着水平轴旋转角度，初始化位200
/*----------------------------------初始化全局变量----------------------------*/
...
int main(void)
{
  ...
  LCD_String(80, 80, "作者：林楚然", 12, DARKBLUE, WHITE);                          // 显示文字;
  LCD_String(68, 100, "指导老师：叶妮佳", 12, DARKBLUE, WHITE);                     // 显示文字;
...
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<总线电机判断区<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/  
  if (g_yemian_num == 1){                                                            // 页面一中屏幕控制产生的事件处理
